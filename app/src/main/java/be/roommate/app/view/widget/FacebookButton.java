...
/**
 * Created by flo on 28/12/15.
 */
public class FacebookButton extends FacebookButtonBase {
    ...
    private static final String TAG = FacebookButton.class.getName();
    private boolean confirmLogout;
    private String loginText;
    private String logoutText;
    private FacebookButtonProperties properties = new FacebookButtonProperties();
    ...
    static class FacebookButtonProperties {
    ...
    /**
     * Create the FacebookButton by inflating from XML
     *
    ...
    /**
     * Create the FacebookButton by inflating from XML and applying a style.
     *
     ...
     * attempt by the user will fail. The FacebookButton can only be associated with either
     * read permissions or publish permissions, but not both. Calling both
     * setReadPermissions and setPublishPermissions on the same instance of FacebookButton
     ...
     * manage the setting of permissions outside of the FacebookButton class altogether
     ...
     * attempt by the user may fail. The FacebookButton can only be associated with either
     * read permissions or publish permissions, but not both. Calling both
     * setReadPermissions and setPublishPermissions on the same instance of FacebookButton
     * will result in an exception being thrown unless clearPermissions is called in between.
     * <p/>
    ...
    /**
     * Clears the permissions currently associated with this FacebookButton.
     */
    public void clearPermissions() {
    ...
    /**
     * Return the current {@link ToolTipMode} for this FacebookButton
     *
    ...
    private void checkToolTipSettings() {
        switch (toolTipMode) {
            case AUTOMATIC:
                ...
                break;
            case DISPLAY_ALWAYS:
                String toolTipString = getResources().getString(com.facebook.R.string.com_facebook_tooltip_default);
                displayToolTip(toolTipString);
                break;
            case NEVER_DISPLAY:
                break;
    ...
    void setProperties(FacebookButtonProperties properties) {
    ...
    protected void configureButton(
        ...
        parseFacebookButtonAttributes(context, attrs, defStyleAttr, defStyleRes);
    ...
    private void parseFacebookButtonAttributes(
            final Context context,
            final AttributeSet attrs,
            final int defStyleAttr,
            final int defStyleRes) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int width;
        if (text == null) {
            text = resources.getString(com.facebook.R.string.com_facebook_loginview_log_in_button_long);
            logInWidth = measureButtonWidth(text);
            width = resolveSize(logInWidth, widthMeasureSpec);
            if (width < logInWidth) {
                text = resources.getString(com.facebook.R.string.com_facebook_loginview_log_in_button);
            }
        }
        logInWidth = measureButtonWidth(text);
        ...
        text = logoutText;
        if (text == null) {
            text = resources.getString(com.facebook.R.string.com_facebook_loginview_log_out_button);
        }
    ...
    private void setButtonText() {
        ...
        if (!isInEditMode() && AccessToken.getCurrentAccessToken() != null) {
            setText((logoutText != null) ?
                    logoutText :
                    resources.getString(com.facebook.R.string.com_facebook_loginview_log_out_button));
        } else {
            if (loginText != null) {
                setText(loginText);
            } else {
                String text = resources.getString(
                        com.facebook.R.string.com_facebook_loginview_log_in_button_long);
                int width = getWidth();
        ...
        public void onClick(View v) {
            ...
            if (accessToken != null) {
                ...
                if (LoginAuthorizationType.PUBLISH.equals(properties.authorizationType)) {
                    if (FacebookButton.this.getFragment() != null) {
                        loginManager.logInWithPublishPermissions(
                                FacebookButton.this.getFragment(),
                                properties.permissions);
                    } else {
                        loginManager.logInWithPublishPermissions(
                                FacebookButton.this.getActivity(),
                                properties.permissions);
                    }
                } else {
                    if (FacebookButton.this.getFragment() != null) {
