...
import be.roommate.app.view.listAdapter.CommentListAdapter;

public class CommentLogActivity extends ActionBarActivity implements RequestActionInterface {
    ...
    protected void save() {
        ...
        if (comment != null && comment.length() > 0) {
            ...
            WebClient<CommentDTO> webClient = new WebClient<>(
                    commentType.requestEnum,
                    commentDTO,
                    CommentDTO.class);
            webClient.setParams("associateId",associateId+"");

            //send request
            Request request = new Request(this,this, webClient);
    ...
    public class ReadRequest implements RequestActionInterface {
        ...
        public ReadRequest() {
            ...
            //build read request
            WebClient<ResultDTO> webClient = new WebClient<>(commentType.getRequestEnumRead(), ResultDTO.class);
            webClient.setParams("commentId",associateId+"");

            Request request = new Request(CommentLogActivity.this,this, webClient);
